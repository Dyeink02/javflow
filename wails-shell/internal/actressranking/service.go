...
var sourceChannels = map[string]sourceChannel{
	...
	// dmm is accepted as a legacy request by normalizeRankingChannel, but is no
	// longer exposed as a separate user-facing source.
...
// Atlas. The AVfan lane contains separately captured, verified 100-row
// snapshots for 2026-03 through 2026-07. The official lane remains distinct
// and contains only its separately captured official rows.
func getBundledInitialCache() cacheFile {
	...
	persistMonthly(&cache, "official", Result{
		...
		Mode: "monthly", PeriodLabel: "2026年07月（本地官方快照）", PeriodYear: 2026, PeriodMonth: 7,
	...
	mergeBundledAnnualHistory(&cache)
	return cache
}

// removeMislabelledOfficialSnapshots removes only cache entries generated by a
// previous build which copied AVfan records into the official bucket. The exact
// marker prevents independently captured official and user data from changing.
func removeMislabelledOfficialSnapshots(cache *cacheFile) bool {
	if cache == nil {
		return false
	}
	official := getSourceBucket(*cache, "official")
	removed := false
	for key, entry := range official.MonthlyByPeriod {
		if strings.TrimSpace(entry.Data.SourceName) != "FANZA 官方（本地 Top100 快照）" {
			continue
		}
		delete(official.MonthlyByPeriod, key)
		removed = true
	}
	if !removed {
		return false
	}
	official.MonthlyLatestKey = ""
	for key := range official.MonthlyByPeriod {
		if official.MonthlyLatestKey == "" || key > official.MonthlyLatestKey {
			official.MonthlyLatestKey = key
		}
	}
	cache.Sources["official"] = official
	return true
}
...
func loadCache(filePath string) cacheFile {
	...
	if err := json.Unmarshal(payload, &current); err == nil && len(current.Sources) > 0 {
		...
		removedMislabelledOfficialSnapshot := removeMislabelledOfficialSnapshots(&current)
		...
		if removedMislabelledOfficialSnapshot || loadedVersion < cacheVersion || !hasBundledBootstrapPeriods(current) {
			_ = writeCache(trimmedPath, skeleton)
		}
		return skeleton
	}
...
func hasBundledBootstrapPeriods(cache cacheFile) bool {
	official := getSourceBucket(cache, "official")
	avfan := getSourceBucket(cache, "avfan")
	officialJuly, officialOK := official.MonthlyByPeriod["2026-07"]
	_, avfanOK := avfan.MonthlyByPeriod["2026-03"]
	return officialOK && len(officialJuly.Data.Items) >= expectedRankingTotal && avfanOK
}
...
func normalizeRankingChannel(channel string) string {
	...
	// Older renderer bundles may still submit dmm. FANZA is the only official
	// option now, so retain request compatibility without retaining a duplicate
	// user-visible source.
	if normalized == "dmm" {
		return "fanza"
	}
...
// getSourceLabel maps internal source/bucket identifiers to readable log text.
func getSourceLabel(sourceID string) string {
	switch sourceID {
	case "official":
		return "FANZA 官方"
	case "avfan":
		return "AVfan"
	case "local", "localHistory":
		return "本地历史"
	default:
		return getChannelLabel(sourceID)
	}
}
...
func normalizeResultMetadata(data Result) Result {
	data.Total = len(data.Items)
	if data.ExpectedTotal <= 0 {
		data.ExpectedTotal = expectedRankingTotal
	}
	data.Complete = data.ExpectedTotal > 0 && data.Total >= data.ExpectedTotal
	return data
}

func emitRankingLog(logFn RankingLogFunc, level string, message string, details map[string]any) {
	if logFn == nil {
		return
	}
	logFn(strings.TrimSpace(level), strings.TrimSpace(message), details)
}

const sourceFailureCooldown = 5 * time.Minute

func (s *Service) sourceIsCoolingDown(sourceID string) (time.Duration, bool) {
	if s == nil {
		return 0, false
	}
	s.healthMu.Lock()
	defer s.healthMu.Unlock()
	health := s.health[sourceID]
	remaining := time.Until(health.UnavailableTil)
	return remaining, remaining > 0
}

func (s *Service) recordSourceResult(sourceID string, failed bool) {
	if s == nil || sourceID == "local" {
		return
	}
	s.healthMu.Lock()
	defer s.healthMu.Unlock()
	...
	if !failed {
		delete(s.health, sourceID)
		return
	}
	health := s.health[sourceID]
	health.Failures++
	// Require two consecutive hard failures before cooling a source. This keeps
	// one transient page error retryable while still avoiding repeated long
	// browser timeouts on a persistently bad proxy.
	if health.Failures >= 2 {
		health.UnavailableTil = time.Now().Add(sourceFailureCooldown)
	}
	s.health[sourceID] = health
}
...
func listAnnualEntries(cache cacheFile, bucketIDs []string) []cachedAnnual {
	...
	for _, bucketID := range bucketIDs {
		...
		for yearKey, entry := range bucket.AnnualByYear {
			...
			if err != nil || !isPublishedAnnualYear(year) || strings.TrimSpace(entry.Data.Title) == "" {
				continue
			}
...
// isPublishedAnnualYear keeps the still-running calendar year out of annual
// selection, even if an old cache or source navigation lists it prematurely.
func isPublishedAnnualYear(year int) bool {
	return year >= 2000 && year < time.Now().Year()
}
...
// offered by the month selector. Keep the union of real cached periods when
// switching sources so the selector cannot collapse to the current official
// month. The request itself still resolves and labels its actual source.
...
func getAnnualAvailability(cache cacheFile, bucketIDs []string) []int {
	...
	for _, item := range listAnnualEntries(cache, bucketIDs) {
		...
		values = append(values, item.Entry.Data.AvailableYears...)
	}
	available := make([]int, 0, len(values))
	for _, year := range normalizeYearList(values) {
		if isPublishedAnnualYear(year) {
...
func persistAnnual(cache *cacheFile, bucketID string, data Result) {
	if cache == nil || !isPublishedAnnualYear(data.PeriodYear) {
		return
	}
	bucket := getSourceBucket(*cache, bucketID)
...
func parseAVFanRankingHTML(htmlSource string, mode string, sourceURL string, fallbackYear int) (Result, error) {
	...
	// 月榜页用 <li> 列表项、年榜页用 <tr> 表格行承载同一套
	// 「名次 <b> + /actress/ 链接 + 头像」结构；<tr> 只有带
	// ranking-cnt 名次标记的行才参与解析，避免吃进导航表格。
	rowNodes := findElementsWithClass(root, "li")
	for _, trNode := range findElementsWithClass(root, "tr") {
		hasRankMarker := findFirst(trNode, func(node *html.Node) bool {
			return node.Type == html.ElementNode &&
				strings.EqualFold(node.Data, "div") &&
				hasAllClasses(node, "ranking-cnt")
		}) != nil
		if hasRankMarker {
			rowNodes = append(rowNodes, trNode)
		}
	}
	for _, listNode := range rowNodes {
...
func getOfficialSourceName(requestedChannel string) string {
	// The official lane is presented as FANZA throughout the current UI. Keep
	// this helper tolerant of legacy request values without reviving DMM as a
...
func (s *Service) fetchOfficialMonthlyRanking(proxyValue string, requestedChannel string, logFns ...RankingLogFunc) (Result, error) {
	var logFn RankingLogFunc
	if len(logFns) > 0 {
		logFn = logFns[0]
	}
	pageURLs := make([]string, 0, 5)
	for page := 1; page <= 5; page++ {
		pageURLs = append(pageURLs, officialMonthlyPageURL(page))
	}
	emitRankingLog(logFn, "info", "官方月榜开始抓取：共 5 页，每页最多 20 位。", map[string]any{"stage": "source.fetch.start", "totalPages": 5})
	startedAt := time.Now()
	emitRankingLog(logFn, "info", "官方榜单年龄验证在共享浏览器会话中开始。", map[string]any{"stage": "age-check.start"})
	pages, err := s.browser.fetchOfficialRankingPages(pageURLs, proxyValue, func(index int, capture officialPageCapture, pageErr error) {
		level := "info"
		message := fmt.Sprintf("官方月榜第 %d/5 页页面加载完成（%.1fs）。", index+1, capture.Duration.Seconds())
		if pageErr != nil {
			level = "warn"
			message = fmt.Sprintf("官方月榜第 %d/5 页页面加载失败：%s。", index+1, getErrorMessage(pageErr))
		}
		emitRankingLog(logFn, level, message, map[string]any{"stage": "page.fetched", "page": index + 1, "totalPages": 5, "url": pageURLs[index]})
	})
	if err != nil {
		emitRankingLog(logFn, "error", fmt.Sprintf("官方月榜抓取失败（%.1fs）：%s。", time.Since(startedAt).Seconds(), getErrorMessage(err)), map[string]any{"stage": "source.fetch.error"})
		return Result{}, createRankingError("官方榜单暂时不可用，请确认日本地区代理 / VPN 和网络连接状态。", "official_unavailable")
	}
	emitRankingLog(logFn, "info", fmt.Sprintf("官方榜单年龄验证完成，共 5 页到位（%.1fs）。", time.Since(startedAt).Seconds()), map[string]any{"stage": "age-check.done"})
	if len(pages) == 0 {
		return Result{}, createRankingError("官方榜单未返回页面内容。", "official_empty")
	}
	htmlSource, pageURL, title := pages[0].HTML, pages[0].PageURL, pages[0].Title
	...
	emitRankingLog(logFn, "info", fmt.Sprintf("官方月榜第 1/5 页解析完成：%d 位。", len(first.Items)), map[string]any{"stage": "page.parsed", "page": 1, "totalPages": 5, "parsed": len(first.Items), "merged": len(first.Items)})
	...
	for page := 2; page <= 5 && page <= len(pages); page++ {
		pageHTML, pageLocation, pageTitle := pages[page-1].HTML, pages[page-1].PageURL, pages[page-1].Title
		...
		emitRankingLog(logFn, "info", fmt.Sprintf("官方月榜第 %d/5 页解析完成：%d 位，累计 %d 位。", page, len(parsed.Items), len(itemsByRank)), map[string]any{"stage": "page.parsed", "page": page, "totalPages": 5, "parsed": len(parsed.Items), "merged": len(itemsByRank)})
	}
...
func (s *Service) fetchOfficialRentalAnnualRanking(year int, proxyValue string, requestedChannel string, logFns ...RankingLogFunc) (Result, error) {
	var logFn RankingLogFunc
	if len(logFns) > 0 {
		logFn = logFns[0]
	}
	...
	pageURLs := make([]string, 0, 5)
	for page := 1; page <= 5; page++ {
		pageURLs = append(pageURLs, officialRentalAnnualPageURL(year, page))
	}
	emitRankingLog(logFn, "info", fmt.Sprintf("官方租赁年榜 %d 年开始抓取：共 5 页。", year), map[string]any{"stage": "source.fetch.start", "totalPages": 5, "year": year})
	startedAt := time.Now()
	emitRankingLog(logFn, "info", "官方年榜年龄验证在共享浏览器会话中开始。", map[string]any{"stage": "age-check.start", "year": year})
	pages, batchErr := s.browser.fetchOfficialRankingPages(pageURLs, proxyValue, func(index int, capture officialPageCapture, pageErr error) {
		level := "info"
		message := fmt.Sprintf("官方租赁年榜第 %d/5 页页面加载完成（%.1fs）。", index+1, capture.Duration.Seconds())
		if pageErr != nil {
			level = "warn"
			message = fmt.Sprintf("官方租赁年榜第 %d/5 页页面加载失败：%s。", index+1, getErrorMessage(pageErr))
		}
		emitRankingLog(logFn, level, message, map[string]any{"stage": "page.fetched", "page": index + 1, "totalPages": 5, "url": pageURLs[index], "year": year})
	})
	if batchErr != nil {
		emitRankingLog(logFn, "error", fmt.Sprintf("官方租赁年榜抓取失败（%.1fs）：%s。", time.Since(startedAt).Seconds(), getErrorMessage(batchErr)), map[string]any{"stage": "source.fetch.error", "year": year})
		return Result{}, createRankingError("官方租赁年榜暂时不可用，请确认日本地区代理或 VPN 连接。", "official_rental_annual_unavailable")
	}
	emitRankingLog(logFn, "info", fmt.Sprintf("官方年榜年龄验证完成，共 5 页到位（%.1fs）。", time.Since(startedAt).Seconds()), map[string]any{"stage": "age-check.done", "year": year})
	for page := 1; page <= 5; page++ {
		targetURL := officialRentalAnnualPageURL(year, page)
		if page > len(pages) {
			return Result{}, createRankingError("官方租赁年榜页面数量不足。", "official_rental_annual_incomplete")
		}
		htmlSource, pageURL, title := pages[page-1].HTML, pages[page-1].PageURL, pages[page-1].Title
		if strings.Contains(pageURL, "not-available-in-your-region") {
			return Result{}, createRankingError("当前线路被 DMM/FANZA 限制，请确认已开启日本地区代理或 VPN。", "official_region_blocked")
		}
		if isAgeCheckPage(pageURL, htmlSource, title) {
		...
		emitRankingLog(logFn, "info", fmt.Sprintf("官方租赁年榜第 %d/5 页解析完成：%d 位，累计 %d 位。", page, len(parsed.Items), len(itemsByRank)), map[string]any{"stage": "page.parsed", "page": page, "totalPages": 5, "parsed": len(parsed.Items), "merged": len(itemsByRank), "year": year})
	}
...
func (s *Service) getAVFanResult(context rankingContext) (Result, error) {
	...
	if context.Mode == "monthly" {
		...
		if !context.ForceRefresh && cached != nil {
			...
			result := decorateMonthlyResult(context.Cache, []string{bucketID}, cached.Entry.Data, context.RequestedChannel, "avfan", true, stale, notice, "", false)
			emitRankingLog(context.Log, "info", fmt.Sprintf("AVfan 月榜缓存命中：%d 位。", result.Total), map[string]any{"stage": "cache.hit", "source": "avfan", "actualCount": result.Total, "stale": stale})
			return result, nil
		}
	...
	if !context.ForceRefresh && cachedAnnual != nil {
		...
		result := decorateAnnualResult(context.Cache, []string{bucketID}, cachedAnnual.Entry.Data, context.RequestedChannel, "avfan", true, stale, notice, "", false)
		emitRankingLog(context.Log, "info", fmt.Sprintf("AVfan 年榜缓存命中：%d 位。", result.Total), map[string]any{"stage": "cache.hit", "source": "avfan", "actualCount": result.Total, "stale": stale})
		return result, nil
	}
...
func (s *Service) getOfficialResult(context rankingContext) (Result, error) {
	...
	if context.Mode == "annual" {
		...
		if !context.ForceRefresh && cachedAnnual != nil {
			...
			result := decorateAnnualResult(context.Cache, []string{bucketID}, cachedAnnual.Entry.Data, context.RequestedChannel, effectiveRequestedChannel, true, stale, notice, "", false)
			emitRankingLog(context.Log, "info", fmt.Sprintf("官方年榜缓存命中：%d 位。", result.Total), map[string]any{"stage": "cache.hit", "source": "official", "actualCount": result.Total, "stale": stale})
			return result, nil
		}

		data, err := s.fetchOfficialRentalAnnualRanking(context.Year, context.Proxy, effectiveRequestedChannel, context.Log)
		if err == nil {
			context.Cache = s.commitRankingCache(context.Cache, context.CacheFilePath, bucketID, data)
	...
	if !context.ForceRefresh && cached != nil {
		...
		result := decorateMonthlyResult(context.Cache, []string{bucketID}, cached.Entry.Data, context.RequestedChannel, effectiveRequestedChannel, true, stale, notice, "", false)
		emitRankingLog(context.Log, "info", fmt.Sprintf("官方月榜缓存命中：%d 位。", result.Total), map[string]any{"stage": "cache.hit", "source": "official", "actualCount": result.Total, "stale": stale})
		return result, nil
	}

	data, err := s.fetchOfficialMonthlyRanking(context.Proxy, effectiveRequestedChannel, context.Log)
	if err == nil {
		context.Cache = s.commitRankingCache(context.Cache, context.CacheFilePath, bucketID, data)
		latestKey := getMonthKey(data.PeriodYear, data.PeriodMonth)
		if requestedKey != "" && requestedKey != latestKey {
...
func buildSourcePlan(requestedChannel string, mode string) []string {
	switch normalizeRankingChannel(requestedChannel) {
	case "local":
		return []string{"local"}
	case "avfan":
		return []string{"avfan", "local"}
	case "fanza":
		// Do not silently substitute an AVfan record here: a source choice must
		// stay truthful. Historical periods can still explicitly fall back to
		// the local-history view and will be labeled as such.
		return []string{"official", "local"}
	default:
		if mode == "monthly" {
...
func enrichFallbackNotice(requestedChannel string, attemptedSource string, result Result) string {
	...
	if normalizeRankingChannel(requestedChannel) == "fanza" && attemptedSource == "official" {
		return mergeNotice(messages.OfficialFallbackTo(result.ResolvedSourceLabel), result.Notice)
	}
	if normalizeRankingChannel(requestedChannel) == "fanza" && result.Mode == "annual" {
...
// AnnualBackfillResult reports one year fetched during a history backfill.
type AnnualBackfillResult struct {
	Year  int           `json:"year"`
	Total int           `json:"total"`
	Items []RankingItem `json:"items"`
}

// BackfillAnnualHistory fills the most recent completed annual rankings. It
// uses the official rental list first and falls back to AVfan when official
// historical pages have been removed. Progress lines are emitted through
// options.Log / the service emitter so the renderer activity log shows live
// status.
func (s *Service) BackfillAnnualHistory(options Options, yearsBack int) ([]Result, error) {
	if s == nil {
		return nil, fmt.Errorf("actress ranking service is not initialized")
	}
	if options.Log == nil && s.emitter != nil {
		options.Log = func(level string, message string, details map[string]any) {
			...
			s.emitter.Emit("atlas:ranking:log", payload)
		}
	}
	if yearsBack <= 0 {
		yearsBack = 5
	...
	fetched := make([]Result, 0, yearsBack)
	for offset := 1; offset <= yearsBack; offset++ {
		year := currentYear - offset
		alreadyCached := false
		for _, bucketID := range []string{"official", "avfan", "localHistory"} {
			bucket := getSourceBucket(cache, bucketID)
			if entry, ok := bucket.AnnualByYear[strconv.Itoa(year)]; ok && len(entry.Data.Items) >= 100 {
				alreadyCached = true
				break
			}
		}
		if alreadyCached {
			emitRankingLog(options.Log, "info", fmt.Sprintf("%d 年榜已有完整缓存（100 位），跳过。", year), map[string]any{
				"stage": "backfill.skip", "year": year,
			})
			continue
		}
		emitRankingLog(options.Log, "info", fmt.Sprintf("开始回填 %d 年榜（优先官方，历史页缺失时使用 AVfan）。", year), map[string]any{
			"stage": "backfill.start", "year": year,
		})
		bucketID := "official"
		data, err := s.fetchOfficialRentalAnnualRanking(year, strings.TrimSpace(options.Proxy), requestedChannel)
		if err != nil {
			emitRankingLog(options.Log, "warn", fmt.Sprintf("%d 官方年榜不可用，尝试 AVfan 历史页：%s。", year, getErrorMessage(err)), map[string]any{
				"stage": "backfill.official-missing", "year": year, "error": getErrorMessage(err),
			})
			data, _, err = s.fetchAVFanAnnualRanking(year, strings.TrimSpace(options.Proxy))
			bucketID = "avfan"
			if err != nil {
				emitRankingLog(options.Log, "warn", fmt.Sprintf("%d 年榜回填失败：%s。", year, getErrorMessage(err)), map[string]any{
					"stage": "backfill.error", "year": year, "error": getErrorMessage(err),
				})
				continue
			}
		}
		cache = s.commitRankingCache(cache, options.CacheFilePath, bucketID, data)
		fetched = append(fetched, data)
		emitRankingLog(options.Log, "success", fmt.Sprintf("%d 年榜回填完成：%d 位。", year, len(data.Items)), map[string]any{
			"stage": "backfill.done", "year": year, "total": len(data.Items),
		})
	}
	return fetched, nil
}
...
func (s *Service) GetActressRankings(options Options) (Result, error) {
	if options.Log == nil && s != nil && s.emitter != nil {
		options.Log = func(level string, message string, details map[string]any) {
			payload := map[string]any{
				"level":     strings.TrimSpace(level),
				"message":   strings.TrimSpace(message),
	...
	cache := loadCache(options.CacheFilePath)
	mergeHistoryDirectoriesIntoCache(&cache, options.HistoryDirectories)
	emitRankingLog(options.Log, "info", fmt.Sprintf("历史榜单缓存已准备：月榜 %d 条，年榜 %d 条。", len(listMonthlyPeriods(cache, []string{"official", "avfan", "localHistory"})), len(listAnnualEntries(cache, []string{"official", "avfan", "localHistory"}))), map[string]any{
		"stage": "cache.ready", "monthlyEntries": len(listMonthlyPeriods(cache, []string{"official", "avfan", "localHistory"})), "annualEntries": len(listAnnualEntries(cache, []string{"official", "avfan", "localHistory"})),
	})
	...
	if mode == "annual" && context.Year > 0 && !isPublishedAnnualYear(context.Year) {
		message := fmt.Sprintf("%d 年榜尚未发布，当前仅可查看已结束年份的年榜。", context.Year)
		emitRankingLog(context.Log, "warn", message, map[string]any{"stage": "annual.unpublished", "year": context.Year})
		return Result{}, createRankingError(message, "annual_unpublished")
	}
	emitRankingLog(context.Log, "info", "榜单请求开始。", map[string]any{
		"mode": mode, "source": requestedChannel, "year": context.Year, "month": context.Month,
	...
	if requestedChannel == "smart" && mode == "monthly" && !context.ForceRefresh && getMonthKey(context.Year, context.Month) != "" {
		...
		if cached := resolveCachedMonthlyEntry(cache, buckets, context.Year, context.Month, true); cached != nil {
			emitRankingLog(context.Log, "info", fmt.Sprintf("命中历史缓存：%04d-%02d，共 %d 位。", context.Year, context.Month, len(cached.Entry.Data.Items)), map[string]any{
				"stage": "cache.hit", "fromCache": true, "actualCount": len(cached.Entry.Data.Items),
			})
			...
			emitRankingLog(context.Log, "info", fmt.Sprintf("榜单请求完成：缓存 %d 位，耗时 %dms。", result.Total, time.Since(startedAt).Milliseconds()), map[string]any{
				"stage": "ranking.complete", "fromCache": true, "actualCount": result.Total, "elapsedMs": time.Since(startedAt).Milliseconds(),
			})
			return result, nil
		}
	}
	// FANZA's official page exposes only the current month. When a user picks a
	// verified historical month that the official bucket does not contain, do
	// not spend time fetching today's page only to discover it cannot answer
	// the request. Return the exact local record immediately and preserve that
	// distinction in both resolved source and source name.
	if requestedChannel == "fanza" && mode == "monthly" && !context.ForceRefresh && getMonthKey(context.Year, context.Month) != "" {
		official := resolveCachedMonthlyEntry(cache, []string{"official"}, context.Year, context.Month, true)
		if official == nil {
			if cached := resolveCachedMonthlyEntry(cache, []string{"localHistory", "avfan"}, context.Year, context.Month, true); cached != nil {
				periodKey := getMonthKey(context.Year, context.Month)
				result := decorateMonthlyResult(
					cache,
					[]string{"localHistory", "avfan"},
					cached.Entry.Data,
					requestedChannel,
					"local",
					true,
					!isFresh(cached.Entry, monthlyCacheMaxAgeMS),
					fmt.Sprintf("FANZA 官方暂未保存 %s 的历史月榜，已立即显示可核验的本地历史。", periodKey),
					"",
					true,
				)
				emitRankingLog(context.Log, "info", fmt.Sprintf("FANZA 历史月榜命中本地记录：%s，共 %d 位。", periodKey, result.Total), map[string]any{
					"stage": "cache.history-fallback", "fromCache": true, "actualCount": result.Total,
				})
				return result, nil
			}
		}
	...
	for _, sourceID := range buildSourcePlan(requestedChannel, mode) {
		if sourceID != "local" && !context.ForceRefresh {
			if remaining, cooling := s.sourceIsCoolingDown(sourceID); cooling {
				emitRankingLog(context.Log, "warn", fmt.Sprintf("数据源 %s 处于失败冷却中，剩余 %ds，跳过并尝试备用源。", getSourceLabel(sourceID), int(remaining.Seconds())), map[string]any{
					"stage": "source.cooldown", "source": sourceID, "remainingMs": remaining.Milliseconds(),
				})
				continue
			}
		}
		emitRankingLog(context.Log, "info", fmt.Sprintf("开始尝试数据源：%s。", getSourceLabel(sourceID)), map[string]any{
			"stage": "source.start", "source": sourceID,
		})
		result, err := s.trySource(sourceID, context)
		if err != nil {
			s.recordSourceResult(sourceID, true)
			emitRankingLog(context.Log, "warn", fmt.Sprintf("数据源 %s 失败：%s。", getSourceLabel(sourceID), getErrorMessage(err)), map[string]any{
				"stage": "source.error", "source": sourceID, "error": getErrorMessage(err),
			})
		...
		s.recordSourceResult(sourceID, false)
		...
		emitRankingLog(context.Log, "success", fmt.Sprintf("榜单请求完成：%s，共 %d 位，耗时 %dms。", result.PeriodLabel, result.Total, time.Since(startedAt).Milliseconds()), map[string]any{
			"stage": "ranking.complete", "source": result.ResolvedSource, "actualCount": result.Total, "complete": result.Complete, "fromCache": result.FromCache, "elapsedMs": time.Since(startedAt).Milliseconds(),
		})
		return result, nil
	}
	...
	emitRankingLog(context.Log, "error", fmt.Sprintf("所有榜单数据源均失败：%s。", message), map[string]any{
		"stage": "ranking.error", "error": message, "elapsedMs": time.Since(startedAt).Milliseconds(),
	})
	return Result{}, createRankingError(message, "ranking_all_sources_failed")
}

...
class CloudflareBypass {
  ...
  constructor(config: CloudflareConfig = {}) {
    this.config = {
      ...
      timeout: Math.max(config.timeout || 45000, 60000), // Cloudflare 挑战页需要更长时间，最低 60 秒
    ...
    try {
      ...
      const cookiesToSet = [
        ...
        },
        {
          ...
          domain: '.javbus.com',
          path: '/',
          expires: (new Date().getTime() / 1000) + (365 * 24 * 60 * 60) // 1年后过期
        },
        {
          name: 'adult_verified',
          value: 'true',
  ...
  public async bypassCloudflare(url: string): Promise<string> {
    ...
    // 在真正访问页面前，确保年龄认证 Cookie 已设置（共享池模式下 init 时还没有 page）
    if (this.page) {
      try {
        await this.setAgeVerificationCookies();
      } catch (cookieError) {
        logger.warn(`bypassCloudflare: 设置年龄认证 Cookie 失败，继续尝试访问页面: ${cookieError instanceof Error ? cookieError.message : String(cookieError)}`);
      }
    }

    try {
      logger.info(`正在访问页面: ${url}`);
      const navTimeout = Math.max(this.config.timeout || 60000, 60000);
      logger.debug(`页面访问参数: 优先 waitUntil=domcontentloaded, timeout=${navTimeout}`);

      // 记录开始时间，用于计算实际耗时
      const pageAccessStartTime = Date.now();

      // 访问页面 - 优先使用 domcontentloaded 避免慢资源导致超时；失败再回退到 load
      ...
      try {
        ...
        logger.debug(`[TIMING] domcontentloaded 完成，耗时: ${Date.now() - pageAccessStartTime}ms`);
      } catch (navError) {
        logger.warn(`domcontentloaded 导航失败，尝试回退到 load: ${navError instanceof Error ? navError.message : String(navError)}`);
        response = await this.page.goto(url, {
          waitUntil: 'load',
          timeout: navTimeout
        });
        logger.debug(`[TIMING] load 完成，耗时: ${Date.now() - pageAccessStartTime}ms`);
      }
      if (!response) {
  ...
  public async getCookies(): Promise<string> {
    ...
    try {
      ...
      // 确保包含年龄验证相关的cookies
      const ageVerificationCookies = ['age', 'dv', 'age_verified', 'adult_verified', 'age_verification_passed', 'is_adult', 'verified_adult'];
      ...
      if (ageButtonClicked) {
        ...
        if (stillOnAgeVerification) {
          ...
          await this.page.evaluate(() => {
            // 尝试设置常见的年龄认证Cookie
            document.cookie = 'age=verified; path=/; max-age=31536000';
            document.cookie = 'dv=1; path=/; max-age=31536000';
  ...
  public async saveCookies(filename: string = 'cloudflare_cookies.json'): Promise<void> {
    ...
    async close(): Promise<void> {
        try {
            // 如果当前实例来自共享池，必须归还而不是关闭，否则池会逐渐枯竭。
            if (this.puppeteerPool && this.currentInstance) {
                logger.debug(`close: 归还共享池实例 ${this.currentInstance.id}`);
                this.puppeteerPool.releaseInstance(this.currentInstance);
                this.currentInstance = null;
                this.browser = null;
                this.page = null;
                logger.info('CloudflareBypass: 已归还共享池实例');
                return;
            }
            ...
            if (this.browser) {
                ...
                try {
                    ...
                    if (browserProcess) {
                        logger.debug(`close: 浏览器进程PID: ${browserProcess.pid}`);
                    }
                } catch (processError) {
